...
module.exports = function() {
	...
	this.init = function() {
		...
		for (var i = 0; i < config.boardSize; i++) {
			...
			//Spawn resources randomly - this might be really inefficient
			} else if (chance < (config.spawnRate.tree + config.spawnRate.rock + config.spawnRate.berry)) {
				if (chance > (config.spawnRate.tree + config.spawnRate.rock)) {
					stand = 'berry';
				} else if (chance > config.spawnRate.tree) {
					stand = 'rock';
				} else {
					stand = 'tree';
		...
		for (var i = 0; i < npcs.length; i++) {
			...
			switch (npc.state) {
				...
				case 'Gathering':
					...
					if (npc["inventory"][questInfo[npc.quest]["Reward"]] === undefined) {
						npc["inventory"][questInfo[npc.quest]["Reward"]] = 1;
					} else {
